...
class GridVisualization extends React.Component<Props, State> {
    ...
    render() {
        return (
            <div>
                <div className="bg">{this.renderViz()}</div>
                <svg xmlns="http://www.w3.org/2000/svg" className="bg-grid">
                    <defs>
                        <marker id="arrowhead" markerWidth="3" markerHeight="3" refX="0" refY="1.5" orient="auto" fill={ARROW_COLOR}>
                            <polygon points="0 0, 3 1.5, 0 3" />
                        </marker>
                    </defs>
                    {this.props.settings.showArrows ? this.renderArrows() : []}
                </svg>
            </div>
        );
    }
    ...
    renderTile(point: Point, color: string) {
        ...
        return <div key={point.x + "," + point.y} style={style} className={this.tileClass} />;
    }
}
